...
Unfortunately there's an [issue](https://github.com/janeczku/calibre-web/issues/2381) in calibre-web right now that invalidates sessions coming through cloudflare. The solution is to add [a script](https://github.com/janeczku/calibre-web/issues/2422#issuecomment-1174411840) to `custom-cont-init.d` that modifies the program at launch (not a fan). Also apparently amazon's requiring that you verify all emails sent to your kindle, which kinda sucks.
